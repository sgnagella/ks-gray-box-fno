...
The linear terms $u_{xx} + u_{xxxx}$ are solved exactly in Fourier space via ETD-RK4. The nonlinear convection term $\frac{1}{2}(u^2)_x$ is treated as **unknown** and approximated by the FNO, which receives a window of past states as input. We impose further physical structure by posing the discoverable nonlinearity as a flux, conserving momentum and avoiding undesirable divergences generated by the trained FNO.
